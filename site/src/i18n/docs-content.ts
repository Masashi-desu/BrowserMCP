...
roadmap/verification-status|実環境での検証状況|Node test、Pages pipeline、各live browser結果はverification.mdへ分けて記録します。静的siteのdeployだけではlocal MCPを証明できず、loopback CA trust、LNA許可、接続申請、Bridgeでのexact Origin承認が必要です。
...
roadmap/verification-status|真实环境验证状态|Node test、Pages pipeline与各live browser结果分别记录在verification.md。仅部署静态site不能证明local MCP，还需要loopback CA trust、LNA许可、连接申请和Bridge exact Origin批准。
...
roadmap/verification-status|Estado de verificación real|Las pruebas Node, el pipeline Pages y cada resultado de browser se registran por separado en verification.md; el despliegue por sí solo no prueba MCP local.
...
roadmap/verification-status|real-environment verification|Node tests, Pages pipeline और प्रत्येक live browser result verification.md में अलग दर्ज होते हैं; deploy अकेला local MCP प्रमाणित नहीं करता।
...
roadmap/verification-status|حالة التحقق الواقعي|تُسجّل اختبارات Node وPages pipeline ونتائج كل browser منفصلة في verification.md؛ النشر وحده لا يثبت MCP المحلي.
...
roadmap/verification-status|Estado da verificação real|Tests Node, o pipeline Pages e cada resultado de browser são registrados separadamente em verification.md; deploy sozinho não comprova MCP local.
...
roadmap/verification-status|Статус реальной проверки|Node tests, Pages pipeline и каждый результат live browser отдельно записываются в verification.md; один deploy не доказывает local MCP.
...
roadmap/verification-status|Real-environment verification|Node test, Pages pipeline ও প্রতিটি live browser result verification.md-এ আলাদা লেখা হয়; deploy একা local MCP প্রমাণ করে না।

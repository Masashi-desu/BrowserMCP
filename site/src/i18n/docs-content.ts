...
architecture/responsibility-boundaries|責務境界|Bridgeはloopback待受、ステートレスなMCP要求処理、上限付き通知購読、認証、ルーティング、キャンセル、タイムアウト、結果変換を所有します。ブラウザアプリは機能宣言、意味上の入力検証、画面コンテキスト、実行を所有し、MCPクライアントは利用者の意図と承認方針を所有します。アプリ固有の検索や保存、ワークフローをBridgeへ追加してはいけません。
...
getting-started/prerequisites|前提条件|共通BridgeにはNode.js 24以降、npm 11以降、IPv4 loopback、現在のブラウザ、MCP 2026-07-28対応のStreamable HTTPクライアントが必要です。公開HTTPS静的アプリにはブラウザが信頼するIP対応ローカル証明書も必要です。macOSアプリはmacOS 14以降とXcodeを要し、実機検証済みはApple Silicon macOSのみです。
...
configuring-mcp-client/client-entry|共通Bridgeを追加する|browsermcpというMCP server entryを1件作成し、MCP 2026-07-28へ固定したStreamable HTTPのhttp://127.0.0.1:8789/mcpと、Bridge起動ごとに表示されるAuthorization: Bearer形式のMCP tokenを設定します。endpointはPOSTのみのステートレス実装で、initialize、Mcp-Session-Id、GET、DELETEには対応しません。
configuring-mcp-client/verify|機能検出とステートレス要求を確認する|siteをpairした後に2026-07-28で接続するか機能一覧を更新し、各要求がclient envelopeを持つことと、各runtimeの名前空間付きTool、Resource、Promptが表示されることを確認します。機能がなければBridgeのapp一覧とsiteの接続panelを調べます。
...
architecture/responsibility-boundaries|职责边界|Bridge负责loopback监听、无状态MCP请求、有界通知订阅、认证、路由、取消、超时与结果转换；浏览器应用负责能力声明、语义输入校验、页面上下文和执行；MCP客户端负责用户意图与审批。不得把应用特有的搜索、存储或工作流逻辑加入Bridge。
...
getting-started/prerequisites|前置条件|通用Bridge需要Node.js 24+、npm 11+、IPv4 loopback、当前浏览器及支持MCP 2026-07-28的Streamable HTTP客户端；公开HTTPS静态应用还需活动浏览器信任的IP有效本地证书。macOS应用需要macOS 14+与Xcode，目前只有Apple Silicon macOS经过实机验证。
...
configuring-mcp-client/client-entry|添加通用Bridge|只创建一个名为browsermcp的MCP server entry，把Streamable HTTP的http://127.0.0.1:8789/mcp固定为MCP 2026-07-28，并设置每次启动的Authorization: Bearer MCP token。端点仅接受POST且无状态，不支持initialize、Mcp-Session-Id、GET或DELETE。
configuring-mcp-client/verify|验证发现与无状态请求|site配对后用2026-07-28连接或刷新能力发现，确认每个请求携带client envelope，并显示各runtime的命名空间Tool、Resource与Prompt。若能力缺失，应先检查Bridge应用列表和site连接面板。
...
architecture/responsibility-boundaries|Límites de responsabilidad|El Bridge posee requests MCP sin estado, suscripciones acotadas, autenticación, routing, límites, timeout y conversión; la app posee declaraciones, validación semántica y ejecución; el cliente posee intención y aprobación.
...
getting-started/prerequisites|Requisitos previos|Se requieren Node.js 24+, npm 11+, loopback IPv4, navegador actual y cliente Streamable HTTP compatible con MCP 2026-07-28; HTTPS público necesita certificado local confiable y la app nativa requiere macOS 14+ y Xcode.
...
configuring-mcp-client/client-entry|Añadir el Bridge común|Cree una sola entrada browsermcp con Streamable HTTP, fije MCP 2026-07-28, la URL loopback y el MCP bearer de cada arranque. El endpoint solo acepta POST y no admite initialize, Mcp-Session-Id, GET ni DELETE.
configuring-mcp-client/verify|Verificar descubrimiento y requests sin estado|Tras emparejar el sitio, conecte con 2026-07-28 o actualice el descubrimiento y confirme Tools, Resources y Prompts con namespace; si faltan, revise primero las listas del Bridge y el panel del sitio.
...
architecture/responsibility-boundaries|जिम्मेदारी की सीमाएँ|Bridge stateless MCP requests, bounded subscriptions, auth, routing, limits और result conversion का मालिक है; app declarations, semantic validation और execution का; client intent और approval का।
...
getting-started/prerequisites|पूर्वापेक्षाएँ|Node.js 24+, npm 11+, IPv4 loopback, current browser और MCP 2026-07-28 वाला Streamable HTTP client चाहिए; public HTTPS को trusted local certificate और native app को macOS 14+ व Xcode चाहिए।
...
configuring-mcp-client/client-entry|common Bridge जोड़ें|एक browsermcp entry में MCP 2026-07-28, Streamable HTTP loopback URL और हर startup का MCP bearer रखें। Endpoint केवल POST और stateless है; initialize, Mcp-Session-Id, GET या DELETE नहीं हैं।
configuring-mcp-client/verify|discovery और stateless requests जाँचें|Pairing के बाद 2026-07-28 से connect या discovery refresh कर namespaced Tools, Resources और Prompts देखें; missing होने पर Bridge list और site panel पहले जाँचें।
...
architecture/responsibility-boundaries|حدود المسؤولية|يمتلك Bridge طلبات MCP عديمة الحالة والاشتراكات المحدودة والمصادقة والتوجيه والمهل وتحويل النتائج؛ يمتلك التطبيق التصريحات والتحقق الدلالي والتنفيذ؛ ويمتلك العميل النية والموافقة.
...
getting-started/prerequisites|المتطلبات|يلزم Node.js 24+ وnpm 11+ وIPv4 loopback ومتصفح حديث وعميل Streamable HTTP يدعم MCP 2026-07-28؛ يحتاج HTTPS العام شهادة محلية موثوقة، ويحتاج التطبيق الأصلي macOS 14+ وXcode.
...
configuring-mcp-client/client-entry|إضافة Bridge المشترك|أنشئ إدخال browsermcp واحدًا وثبّت MCP 2026-07-28 مع Streamable HTTP وعنوان loopback وMCP bearer لكل تشغيل. تقبل النقطة POST فقط ولا تدعم initialize أو Mcp-Session-Id أو GET أو DELETE.
configuring-mcp-client/verify|التحقق من الاكتشاف والطلبات عديمة الحالة|بعد الاقتران اتصل عبر 2026-07-28 أو حدّث الاكتشاف وتحقق من Tools وResources وPrompts ذات namespace؛ افحص قوائم Bridge ولوحة الموقع أولًا عند غيابها.
...
architecture/responsibility-boundaries|Limites de responsabilidade|O Bridge cuida de requests MCP sem estado, subscriptions limitadas, autenticação, routing, limites, timeout e conversão; o app cuida das declarações, validação semântica e execução; o cliente cuida da intenção e aprovação.
...
getting-started/prerequisites|Pré-requisitos|São necessários Node.js 24+, npm 11+, loopback IPv4, navegador atual e cliente Streamable HTTP compatível com MCP 2026-07-28; HTTPS público exige certificado local confiável e o app nativo exige macOS 14+ e Xcode.
...
configuring-mcp-client/client-entry|Adicionar o Bridge comum|Crie uma única entrada browsermcp com Streamable HTTP, fixe MCP 2026-07-28, a URL loopback e o MCP bearer de cada inicialização. O endpoint aceita apenas POST e não oferece initialize, Mcp-Session-Id, GET ou DELETE.
configuring-mcp-client/verify|Verificar descoberta e requests sem estado|Depois do pareamento, conecte com 2026-07-28 ou atualize a descoberta e confira Tools, Resources e Prompts com namespace; se faltarem, veja primeiro as listas do Bridge e o painel do site.
...
architecture/responsibility-boundaries|Границы ответственности|Bridge отвечает за stateless MCP requests, ограниченные subscriptions, auth, routing, limits, timeout и преобразование; приложение — за объявления, смысловую проверку и выполнение; клиент — за намерение и approval.
...
getting-started/prerequisites|Требования|Нужны Node.js 24+, npm 11+, IPv4 loopback, актуальный браузер и клиент Streamable HTTP с поддержкой MCP 2026-07-28; публичному HTTPS нужен доверенный локальный сертификат, нативному приложению — macOS 14+ и Xcode.
...
configuring-mcp-client/client-entry|Добавление общего Bridge|Создайте одну запись browsermcp со Streamable HTTP, закрепите MCP 2026-07-28, loopback URL и MCP bearer каждого запуска. Endpoint принимает только POST и не поддерживает initialize, Mcp-Session-Id, GET или DELETE.
configuring-mcp-client/verify|Проверка discovery и stateless requests|После pairing подключитесь с 2026-07-28 или обновите discovery и проверьте namespaced Tools, Resources и Prompts; при отсутствии сначала смотрите списки Bridge и панель site.
...
architecture/responsibility-boundaries|দায়িত্বের সীমা|Bridge stateless MCP request, সীমিত subscription, auth, routing, limit, timeout ও conversion-এর মালিক; app declaration, semantic validation ও execution-এর; client intent ও approval-এর।
...
getting-started/prerequisites|পূর্বশর্ত|Node.js 24+, npm 11+, IPv4 loopback, বর্তমান browser ও MCP 2026-07-28 সমর্থিত Streamable HTTP client চাই; public HTTPS-এ trusted local certificate এবং native app-এ macOS 14+ ও Xcode লাগে।
...
configuring-mcp-client/client-entry|Common Bridge যোগ করুন|একটি browsermcp entry-তে MCP 2026-07-28, Streamable HTTP loopback URL ও প্রতি startup-এর MCP bearer দিন। Endpoint শুধু POST গ্রহণ করে এবং initialize, Mcp-Session-Id, GET বা DELETE সমর্থন করে না।
configuring-mcp-client/verify|Discovery ও stateless request যাচাই|Pairing-এর পরে 2026-07-28 দিয়ে connect বা discovery refresh করে namespaced Tools, Resources ও Prompts দেখুন; না থাকলে আগে Bridge list ও site panel দেখুন।
